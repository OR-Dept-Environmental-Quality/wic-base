...
Inputs (stored within the "impactFactors/source" directory of the wic-base repository):
...
(Note: If the "source" folder in your downloaded repository contains only a zip file, it is because the uncompressed files are too large for github version control.  You can populate the "source" folder on your local machine by extracting the zip file into the same folder.)
...
csSelected2 <-
   ...
   mutate(
     # fix that weird Textiles typo
      ...
      disposition =
        ifelse(
          disposition=="recylingToContainer <LC>",
...
Ut oh, I see a little bug.  "recyclingToContainer" appears twice-- once as it should, and another time as "recyclingToContainer <LC>".  gotta fix that.  I'll make a ticket.
