...
However, those finished impact factors are slightly altered from their source expression in GABI life cycle assessment system.  The GABI-based factors are documented in *Impact modeling for the Waste Impact Calculator*.

This R markdown document imports impact factors as expressed in GABI, modifies and filters them, and then saves the results in the "distributable" folder for use in WIC.

Inputs (stored within the "source" directory of the current file tree)

* CSV files with impact factor data
* "gabiExportDate.txt"
...
WIC's impact factors are defined within Peter Canepa's Gabi life cycle assessment software and then exported via Gabi's "pivot table export," which creates extremely large CSV files.  These are stored in the "source" folder within the current folder tree. (Note: these files are too large for version control, so this folder may need to be populated via zip files available in the [wic-base release](https://github.com/OR-Dept-Environmental-Quality/wic-base/releases).)
...
There are some complications along the way.  The most notable is the fact that there are actually 2 different files I need to import...
