...
Inputs (stored within the "source" directory of the impactFactors folder of the wic-base repository):
...
WIC's impact factors are defined within Peter Canepa's Gabi life cycle assessment software and then exported via Gabi's "pivot table export," which creates extremely large CSV files (as of this writing, ~80MB).  These should be found in the "source" folder of the impactFactors folder of the wic-base repository.

(Note: If the "source" folder in your downloaded repository contains only a zip file, it is because the uncompressed files are too large for github version control.  You can populate the "source" folder by extracting the zip file into the same folder.)
